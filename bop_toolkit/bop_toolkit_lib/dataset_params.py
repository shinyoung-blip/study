...
def get_model_params(datasets_path, dataset_name, model_type=None):
  ...
  if dataset_name in obj_ids:
    obj_ids = obj_ids[dataset_name]
    
  else:
    models_info_ = json.load(open(os.path.join(datasets_path, '%s/models/models_info.json'%dataset_name)))
    keys_ = list(models_info_.keys())
    keys_ = np.array(keys_, dtype = 'int')
    obj_ids = np.sort(keys_).tolist()
  ...
  if dataset_name in symmetric_obj_ids:
    symmetric_obj_ids = symmetric_obj_ids[dataset_name]
    
  else:
    symmetric_obj_ids = []
    for obj_id_ in obj_ids:
      keys_obj_ =  models_info_[str(obj_id_)]
      for keys_obj_i in keys_obj_:
        if 'sym' in keys_obj_i:
          symmetric_obj_ids.append(obj_id_)
          break
...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
  ...
  # HOPE.
    ...
    camera_info_ = json.load(open(os.path.join(datasets_path, '%s/camera.json'%dataset_name)))
    p['im_size'] = (camera_info_['width'], camera_info_['height'])
