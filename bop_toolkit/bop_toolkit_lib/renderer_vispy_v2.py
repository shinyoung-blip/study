...
uniform float u_light_diffuse_weight;
uniform float u_light_specular_weight;
uniform float u_light_specular_shininess;
uniform vec3 u_light_eye_pos; // light_position
...
void main() {
    ...
    vec3 light_direction = normalize(u_light_eye_pos - v_eye_pos);
    ...
    if (diffuse > 0.0) {
        ...
        vec3 view_direction = normalize(-v_eye_pos);
        specular = pow(max(dot(reflected, view_direction), 0.0), u_light_specular_shininess);
    }

    vec3 ambient = u_light_ambient_w * u_light_color;
    vec3 diffuse_color = u_light_diffuse_weight * diffuse * u_light_color;
    vec3 specular_color = u_light_specular_weight * specular * u_light_color;

    if(bool(u_use_texture)) {
        gl_FragColor = vec4(texture2D(u_texture, v_texcoord)) * vec4(ambient + diffuse_color + specular_color, 1.0);
    }
    else {
        vec3 final_color = v_color * (ambient + diffuse_color + specular_color);
...
# void main() {
#     // 计算环境光照分量
#     //vec3 ambient_color = u_light_color * u_light_ambient_w;
#     // 计算漫反射光照分量
#     //vec3 light_dir = normalize(u_light_eye_pos);
#     //float diffuse_intensity = dot(v_normal, light_dir);
#     //vec3 diffuse_color = u_light_color * u_light_diffuse_weight * max(diffuse_intensity, 0.0);
#     // 计算高光反射光照分量
#     //vec3 view_dir = normalize(-gl_FragCoord.xyz);
#     //vec3 reflect_dir = reflect(-light_dir, v_normal);
#     //float specular_intensity = pow(max(dot(view_dir, reflect_dir), 0.0), u_light_specular_shininess);
#     //vec3 specular_color = u_light_color * u_light_specular_weight * specular_intensity;
#     // 计算最终的颜色值
...
void main() {
    ...
    if(bool(u_use_texture)) {
        gl_FragColor = vec4(light_w * texture2D(u_texture, v_texcoord)) * vec4(u_light_color, 1.0) * u_light_diffuse_weight;
    }
    else {
        gl_FragColor = vec4(light_w * v_color, 1.0) * vec4(u_light_color, 1.0) * u_light_diffuse_weight;
    }
}
"""
...
class RendererVispy(renderer.Renderer, app.Canvas):
  ...
  def add_object(self, obj_id, model_path, **kwargs):
    ...
    if self.mode == "depth":
      ...
      if self.shading == "flat":
        ...
        vertices = np.array(list(zip(model["pts"], model["normals"].astype(np.float32), colors, texture_uv)), vertices_type)
  ...
  def render_object(self, obj_id, R, t, fx, fy, cx, cy, clear=True, draw_back = False):
    """See base class."""
    ...
    self.update()
    self.on_draw(obj_id, mat_model, mat_view, mat_proj, clear=clear, draw_back = draw_back)
  ...
  def on_draw(self, obj_id, mat_model, mat_view, mat_proj, clear=True, draw_back = False):
    with self.fbo:
      ...
      # # 设置光源位置（位置数据：[x, y, z, w]）
      # light_pos = [0.0, 0.0, -300.0, 0.0]  # 最后一个参数w设为0表示该光源为方向光源
      ...
      if draw_back is True:
        gl.glCullFace(gl.GL_FRONT)  # 剔除正面
      else:
  ...
  def _draw_rgb(self, obj_id, mat_model, mat_view, mat_proj):
    ...
    program["u_light_color"] = self.light_color
    program["u_light_diffuse_weight"] = self.light_diffuse_weight
    program["u_light_specular_weight"] = self.light_specular_weight
    program["u_light_specular_shininess"] = self.light_specular_shininess
