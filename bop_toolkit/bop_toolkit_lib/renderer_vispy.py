...
class SingletonArgs(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        init = cls._init[cls]
        if init is not None:
            callargs_hashable = {hashable(k): hashable(
                v) for k, v in inspect.getcallargs(init, None, *args, **kwargs).items()}
            key = (cls, frozenset(callargs_hashable.items()))
        else:
            key = cls
        if key not in cls._instances:
            cls._instances[key] = super(
                SingletonArgs, cls).__call__(*args, **kwargs)
